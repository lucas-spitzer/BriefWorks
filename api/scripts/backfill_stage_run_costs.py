#!/usr/bin/env python3
"""Backfill api_usage and cost_usd from existing stage_runs.token_usage."""
...
from app.services.stage_run_backfill import backfill_fields_for_stage_run
    ...
    *,
    ...
    if dry_run:
        for item in updated_runs:
            ...
            print(
                f"[dry-run] {item['stage_id']} ({item['id']}): "
    ...
    for item in updated_runs:
        ...
        print(
            f"Updated {item['stage_id']} ({item['id']}): "
            f"${item['billing']['cost_usd']:.6f}",
        )

    for production_run_id, _partial in production_totals.items():
        cost_usd = db.sum_stage_run_costs(production_run_id)
...
def main() -> int:
    ...
    parser.add_argument("--production-run-id", help="Backfill stage runs for one production run")
    parser.add_argument("--workspace-id", help="Backfill all stage runs in a workspace")
    ...
    if args.production_run_id:
        rows = db.list_stage_runs_for_production_run(args.production_run_id)
    else:
        rows = db.list_stage_runs_for_workspace(args.workspace_id)

    if not rows:
        print("No stage runs found.", file=sys.stderr)
        return 1

    backfill_stage_runs(db, rows, dry_run=args.dry_run)
    return 0
