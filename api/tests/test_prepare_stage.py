...
from app.intellex.stages.prepare import PrepareStage
...
def test_pre_filter_removes_front_matter_and_clutter() -> None:
    ...
    assert report["reasons"].get("toc_page", 0) >= 1
...
def test_prepare_stage_excludes_marked_lines() -> None:
    document = ParsedDocument(
        page_count=2,
        lines=[
    ...
    output, execution = stage.run(parsed_document=document)
...
def test_prepare_stage_applies_llm_exclusions_after_pre_filter() -> None:
    document = ParsedDocument(
        page_count=2,
        lines=[
    ...
    output, _ = stage.run(parsed_document=document)
