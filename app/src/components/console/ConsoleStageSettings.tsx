...
function CapabilityMeter({ tier }: { tier: number }) {
  return (
    <span className="bw-console__cap" aria-label={`Capability tier ${tier} of 5`}>
      {[1, 2, 3, 4, 5].map((step) => (
        <span
          key={step}
          className={`bw-console__cap-seg${step <= tier ? ' is-on' : ''}`}
          aria-hidden="true"
        />
      ))}
    </span>
...
export function ConsoleStageSettings() {
  ...
  return (
    <>
      <header className="bw-console__header">
        <div>
          <div className="bw-console__eyebrow">Configuration</div>
          <h2>Stage Models</h2>
        </div>
        <span className="bw-console__live">
          <span className="bw-live-dot" /> {settings.length} stages
        </span>
      </header>
      ...
      <div className="bw-console__scroll">
        ...
        {!workspaceId ? (
          <div className="bw-console__empty">Select a workspace to configure stage models.</div>
        ) : isLoading && settings.length === 0 ? (
          <div className="bw-console__empty">Loading stage settings…</div>
        ) : (
          <div className="bw-console__stage-settings">
            {settings.map((setting) => {
              ...
              return (
                <div className="bw-console__panel bw-console__setting" key={setting.stage_action}>
                  <div className="bw-console__setting-main">
                    <div className="bw-console__setting-icon" aria-hidden="true">
                      <SlidersHorizontal size={16} strokeWidth={1.75} />
                    </div>
                    <div className="bw-console__setting-body">
                      <div className="bw-console__setting-title">
                        {setting.label}
                        <span
                          className={`bw-console__setting-badge${
                      ...
                      <div className="bw-console__setting-meta">
                        {current ? (
                          <>
                  ...
                  <div className="bw-console__setting-control">
                    <select
                      className="bw-console__select"
                      value={setting.model}
                      disabled={isSaving}
                    ...
                    {kind === 'effort' ? (
                      <label className="bw-console__reasoning">
                        <span className="bw-console__reasoning-label">Reasoning effort</span>
                        <select
                          className="bw-console__select bw-console__select--sm"
                      ...
                      <label className="bw-console__reasoning">
                        <span className="bw-console__reasoning-label">Thinking budget</span>
                        <select
                          className="bw-console__select bw-console__select--sm"
                    ...
                    {setting.is_overridden ? (
                      <button
                        type="button"
                        className="bw-console__reset-link"
                        disabled={isSaving}
                      ...
                      <span className="bw-console__setting-default">
