...
> **Superseded.** `extract-chapter-knowledge` has been removed from the
> pipeline and deleted from the codebase. Automated selection tuning (the
> approach below) was replaced by human curation: the author reads the
> generated ebook and curates `wiki_entries` directly through the manual
> authoring flow. See [wiki-authoring-plan.md](wiki-authoring-plan.md) and
> [wiki-authoring-contract.md](wiki-authoring-contract.md). This document is
> kept for historical context on why the old extraction approach was
> insufficient.

How `extract-chapter-knowledge` decided which terms, concepts, and insights
became canonical wiki entries — and how to make that decision deliberate,
calibrated, and bounded. The wiki layer was the sole input to QnGen, so its
selectivity drove every downstream count and quality issue.
